          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {farmerPayments.map((p, i) => (
                <motion.tr key={p.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="border-b border-gray-50">
                  ...
                  <td className="p-4"><span className={`text-xs font-semibold px-2 py-1 ${statusColors[p.status]}`}>{p.status}</span></td>
                </motion.tr>
              ))}
            </tbody>
          </table>
