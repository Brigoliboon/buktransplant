...
import { LayoutDashboard, ClipboardList, FileText, User, LogOut, Menu } from "lucide-react";

const navItems = [
  { label: "Dashboard", href: "/operator/dashboard", icon: LayoutDashboard },
  { label: "Assigned Jobs", href: "/operator/jobs", icon: ClipboardList },
  { label: "Reports", href: "/operator/reports", icon: FileText },
  ...
  return (
    <div className="min-h-screen flex bg-light-bg pt-16">
      <aside className={`fixed md:sticky top-16 left-0 z-30 h-[calc(100vh-4rem)] bg-white border-r border-gray-100 transition-all duration-300 ${sidebarOpen ? "w-64" : "w-0 md:w-64 overflow-hidden md:overflow-visible"}`}>
        <div className="p-4">
          <p className="text-xs text-gray uppercase tracking-wider font-semibold mb-4">Operator Panel</p>
