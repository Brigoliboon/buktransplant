...
const statusColors: Record<string, string> = {
  "on-job": "text-accent bg-accent/10",
        ...
        {branchOperators.map((op, i) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-xs font-semibold px-2 py-1 ${statusColors[op.status]}`}>{op.status}</span>
            </div>
            <p className="text-xs text-gray">Specializes in: {op.specialization}</p>
